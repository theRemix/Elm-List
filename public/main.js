...
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var toListItem = function (x) {
      return A2($Html.li,_U.list([]),_U.list([$Html.text(x)]));
   };
   var chiptuneArtists = _U.list(["Sabrepulse"
                                 ,"Pixelh8"
                                 ,"Big Giant Circles"]);
   var main = A2($Html.ul,
   _U.list([$Html$Attributes.$class("artists")]),
   A2($List.map,toListItem,chiptuneArtists));
   return _elm.Main.values = {_op: _op
                             ,chiptuneArtists: chiptuneArtists
